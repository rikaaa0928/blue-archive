...
function launch(batchId) {
  ...
  child.on("close", exitCode => {
    ...
    writeJsonAtomic(batchPath(batchId), {
      ...latest,
      status: exitCode === 0 ? "waiting" : "failed",
      exitCode,
      pid: null,
      finishedAt: nowIso(),
    });
...
export function createBatch({ query, storyIds, params = {} }) {
  ...
  if (selected.some(storyId => !allowed.has(storyId))) {
    throw new Error("The batch contains a chapter outside the selected event");
  }
  const ordered = series.chapters.filter(chapter => selected.includes(chapter.storyId));
  const batchId = `${Date.now()}-event-${series.id}`;
  fs.mkdirSync(batchDirectory(batchId), { recursive: true });
  writeJsonAtomic(batchPath(batchId), {
    schemaVersion: 2,
    id: batchId,
    kind: "event-series-next-human",
