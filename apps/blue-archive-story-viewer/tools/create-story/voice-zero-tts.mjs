...
const npcReferenceCharacterName = anonymousNpcPresetVoice.characterName;
...
function printUsage() {
  console.log(`Usage:
  node ./tools/create-story/voice-zero-tts.mjs <story-id-or-json-path> [options]
...
Options:
  ...
  --story-id <id>           logical story id for a JSON with a generic filename
  --manifest <file>         workflow manifest path, default under .local-files
  --collective-config <file>
                            reviewed collective/unknown-speaker config override
  ...
  --reference-selection <file>
                            manual speaker-to-clip basename selections from workbench
  ...
  --indices <csv>           process only the specified story indices
  --skip-indices <csv>      exclude reviewed no-voice story indices
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    switch (arg) {
      ...
      case "--directory-id":
        ...
        break;
      case "--story-id":
        args.storyId = readOptionValue(argv, ++index, arg);
        break;
      case "--manifest":
        args.manifest = readOptionValue(argv, ++index, arg);
        break;
      case "--collective-config":
        args.collectiveConfig = readOptionValue(argv, ++index, arg);
        break;
      case "--character-root":
      ...
      case "--reference-selection":
        args.referenceSelection = readOptionValue(argv, ++index, arg);
        break;
      case "--stage":
      ...
      case "--indices": {
        ...
        const indices = value.split(",").map(item => Number(item.trim()));
        if (!indices.length || indices.some(item => !Number.isSafeInteger(item) || item < 0)) {
          throw new Error("--indices must be a comma-separated list of non-negative integers");
        }
      ...
      case "--skip-indices": {
        ...
        args.skipIndices = [...new Set(indices)];
        break;
      }
      case "--reference-min":
...
function resolveStoryId(args, storyPath) {
  const storyId = String(args.storyId || storyIdFromPath(storyPath));
  if (!/^[A-Za-z0-9_-]+$/u.test(storyId)) {
    throw new Error("--story-id must contain only letters, numbers, underscores, or hyphens");
  }
  return storyId;
}
...
function loadCollectiveVoiceConfig(storyPath, story, args) {
  let configPath;
  let relativeStoryPath;
  if (args.collectiveConfig) {
    configPath = path.resolve(process.cwd(), args.collectiveConfig);
    const storyId = resolveStoryId(args, storyPath);
    const flatType = ["main", "other"].includes(args.type);
    relativeStoryPath = path.join(
      "public",
      "story",
      args.type,
      ...(flatType ? [] : [args.directoryId || storyId.slice(0, 5)]),
      `${storyId}.json`,
    ).split(path.sep).join("/");
  } else {
    ({ configPath, relativeStoryPath } = resolveCollectiveConfigPath(storyPath));
  }
...
function manualReferenceSelection(args, speaker) {
  if (args.referenceSelection) {
    args.referenceSelections ||= JSON.parse(
      fs.readFileSync(path.resolve(args.referenceSelection), "utf8"),
    );
    const selectedNames = args.referenceSelections[speaker];
    if (Array.isArray(selectedNames)) return selectedNames.map(String);
  }
  return [];
}
...
function selectReferenceClips(candidates, args, speaker) {
  ...
  if (selectedNames.length > 0) {
    ...
    const selected = selectedNames.map(name => byName.get(String(name)));
...
function prepareReferenceAudio({ args, speaker, characterName, manifest }) {
  ...
  if (
    ...
    const cachedSelection = Array.isArray(cached.clips)
      ? cached.clips.map(clip => String(clip.name))
      : [];
    const selectionMatches = manualSelection.length === 0 || (
      manualSelection.length === cachedSelection.length &&
...
async function apiRequest(args, endpoint, options = {}) {
  ...
  return requestZeroTts({
    baseUrl: args.ttsBaseUrl,
    token,
    endpoint,
    options,
  });
...
async function main() {
  ...
  const collectiveConfig = loadCollectiveVoiceConfig(storyPath, story, args);
  const speakerMap = await loadSpeakerMap(args, story, collectiveConfig);
  const extractedVoiceLines = extractVoiceLines(
    story,
    speakerMap,
    collectiveConfig,
    0,
  ...
  const allVoiceLines = extractedVoiceLines.filter(line => !skippedIndices.has(line.index));
  ...
  if (args.indices.length > 0) {
    const selected = new Set(args.indices);
    voiceLines = allVoiceLines.filter(line => selected.has(line.index));
    const missing = args.indices.filter(index => !voiceLines.some(line => line.index === index));
    if (missing.length) throw new Error(`Requested indices are not voice lines: ${missing.join(", ")}`);
  } else if (args.regenerateCollectiveMember) {
