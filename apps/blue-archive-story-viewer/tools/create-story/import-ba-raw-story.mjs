...
const defaultCnProofreadModel =
  process.env.CN_PROOFREAD_MODEL || "gemini-3.7-flash";
...
function printUsage() {
  console.log(`Usage:
  node ./tools/create-story/import-ba-raw-story.mjs <story-id> [options]
...
Options:
  ...
  --workbench-raw-import supplement source languages, but defer Simplified
                         Chinese normalization, TextJpVoice overrides and LLM
                         review to later versioned workbench stages
...
Examples:
  node ./tools/create-story/import-ba-raw-story.mjs 1103 --type group --dry-run
  node ./tools/create-story/import-ba-raw-story.mjs 1103 --type group --force
  node ./tools/create-story/import-ba-raw-story.mjs 1103 \\
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    switch (arg) {
      ...
      case "--no-cn-llm-proofread":
        ...
        break;
      case "--workbench-raw-import":
        args.workbenchRawImport = true;
        args.useCnLlmProofread = false;
        break;
...
async function main() {
  ...
  const openCc = args.workbenchRawImport
    ? {
      filled: 0,
      mappedNameOccurrences: 0,
      mappedNames: 0,
      missingTextTwOnDisplayTextRows: content.filter(
        row => row.TextJp && !row.TextCn && !row.TextTw,
      ).length,
    }
    : fillMissingTextCnFromTextTw(content, characterNameMappings);
  const normalizedCharacterNames = args.workbenchRawImport
    ? { changedRows: 0, mappedNameOccurrences: 0, mappedNames: 0 }
    : normalizeExistingTextCnCharacterNames(content, characterNameMappings);
  ...
  const summary = {
    ...
    workbenchRawImport: args.workbenchRawImport,
    cnProofread,
    cnProofreadSkipped,
  };
