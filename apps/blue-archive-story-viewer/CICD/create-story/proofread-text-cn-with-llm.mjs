...
async function proofreadStoryTextCnOnePassWithLlm(story, options = {}) {
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    const prompt = buildPrompt({ story, textUnits, glossary, batch, context });
...
function printUsage() {
  console.log(`Usage:
  node ./CICD/create-story/proofread-text-cn-with-llm.mjs <story-json-or-directory> [options]
...
async function main() {
  ...
  const backupRoot = args.dryRun
    ? ""
    : fs.mkdtempSync(path.join(os.tmpdir(), "ba-cn-proofread-"));
  if (backupRoot) {
