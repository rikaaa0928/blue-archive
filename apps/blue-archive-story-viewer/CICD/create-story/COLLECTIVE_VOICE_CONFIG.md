...
5. 只有配置状态完整且验证通过，才可以运行 ZeroTTS 的 `upload`、`tasks`、
   `poll`、`all` 阶段或包含 TTS 的 `process-story`。
...
配置：CICD/create-story/config/collective-voices/event/10014/10014030.json

剧情：public/story/group/1102/1102.json
配置：CICD/create-story/config/collective-voices/group/1102/1102.json

剧情：public/story/other/400.json
配置：CICD/create-story/config/collective-voices/other/400.json
```
...
- 原始 `expected.speaker` 必须确实是由两个以上问号组成的未知说话人。
